    ...
    <>
      <div className="page-heading row-heading">
        <div>
          ...
          <h1>Notifications</h1>
          <p>Workshop updates and announcements, as they happen.</p>
        </div>
        <Action
          className="secondary"
          onClick={async () => {
      ...
      <section className="panel">
        ...
        {!data ? (
            ...
            >
              <div>
                ...
                <Link
                  href={n.href}
                  onClick={() =>
                    void api(`notifications/${n.id}`, {}).then(refresh)
                  }
                >
                  {n.message}
                </Link>
          ...
          <Empty title="You’re all caught up">
            New workshop notifications will appear automatically.
          </Empty>
        )}
      </section>
    </>
...
export function Certificates() {
  const { data, error } = usePoll<Certificate[]>("certificates");
  return (
    <>
      <div className="page-heading">
        <p className="eyebrow">VERIFIED LEARNING</p>
        <h1>Certificates</h1>
        <p>Your achievements, backed by verified workshop attendance.</p>
      </div>
      <ErrorBox message={error} />
      {!data ? (
        <Loading />
            ...
            <article className="panel certificate-card" key={c.id}>
              ...
              <div className="actions">
                ...
                <a
                  ...
                  href={`/api/certificate/${c.id}/pdf?download=1`}
                >
                  <Download size={16} />
                  PDF
                </a>
        ...
        <section className="panel">
          <Empty title="Your certificates will appear here">
            Complete a workshop with at least 90% meeting attendance and
            verified QR attendance. Eligible certificates are issued when the
            organizer ends the session.
          </Empty>
          <Link className="button" href="/workshops">
            Explore workshops
          </Link>
        </section>
      )}
  ...
  return (
    <>
      <div className="page-heading">
        ...
        {c && (
          <p>
            {c.participantName} · {c.workshopTitle}
          </p>
        )}
      </div>
      <ErrorBox message={error} />
        ...
        <>
          <div className="actions certificate-actions">
            <a
              ...
              href={`/api/certificate/${id}/pdf?download=1`}
            >
              <Download size={18} />
              Download PDF
            </a>
...
export function Profile({ user }: { user: User }) {
  ...
  return (
    <>
      <div className="page-heading">
        <h1>Your profile</h1>
        <p>Keep your name correct for future certificates.</p>
      </div>
      <section className="panel form-panel">
        ...
        >
          ...
          {saved && (
            <p className="success">
              Profile saved. Existing issued certificates keep their original
              details.
            </p>
          )}
...
export function Reports() {
  const { data: d, error, refresh } = usePoll<Dashboard>("dashboard");
  const [confirmation, setConfirmation] = useState("");
  return (
    <>
      <div className="page-heading">
        <p className="eyebrow">ADMINISTRATION</p>
        <h1>Reports & system activity</h1>
        ...
        <>
          ...
          {d.demoMode && (
            <section className="panel reset-panel">
              ...
              <p>
                Removes organizers and workshops created in demo mode, plus
                their registrations, attendance, QR challenges, notifications
                and certificates. Seeded Admin and Participant accounts are
                preserved.
              </p>
              <label>
...
export function WorkshopResources({
  ...
  mode: "attendance" | "materials";
}) {
  const { data, error } = usePoll<Workshop[]>("workshops");
  const [selected, setSelected] = useState("");
  return (
    <>
      <div className="page-heading">
        <p className="eyebrow">YOUR WORKSHOP RESOURCES</p>
        <h1>{mode === "attendance" ? "Attendance" : "Learning materials"}</h1>
        <p>
          {mode === "attendance"
            ? "Track verified participation in your workshops."
            : "Open a workshop to access its learning resources."}
        </p>
      </div>
      <ErrorBox message={error} />
      {!data ? (
        ...
        <section className="panel">
          ...
          {selected &&
            (mode === "attendance" ? (
              <AttendanceResource id={selected} />
            ) : (
              <Link className="button" href={`/workshops/${selected}`}>
        ...
        <section className="panel">
          <Empty title="No workshops available" />
        </section>
      )}
    </>
  );
}
function AttendanceResource({ id }: { id: string }) {
      ...
      {data ? (
        <>
          ...
          <div className="actions">
            ...
            <button
              className="button secondary"
              onClick={() => downloadAttendance("workshop", data)}
            >
              Export CSV
            </button>
          </div>
